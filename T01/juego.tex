...
\begin{algorithm}[H]
  ...
  \begin{algorithmic}[1]
    ...
    \For{$jugador\in res.jugadores$}\Comment{$O(K)$}
        ...
        \State\asignar{jugador.cantFichasTotal}{0}
...
\begin{algorithm}[H]
  ...
  \begin{algorithmic}[1]
    ...
    \State\Return false
    \EndIf
    \State\asignar{oIt}{\textsc{CrearIt}(o)}
    \While{$\textsc{HaySiguiente}(oIt)$}\Comment{$O(\Lmax)$}
        ...
        \State\asignar{oItSig}{\textsc{Avanzar}(copy(oIt))}
        \While{$\textsc{HaySiguiente}(oItSig)$}\Comment{$O(\Lmax)$}
          \State\asignar{ficha'}{\textsc{Siguiente}(oItSig)}
          \If{$\pi_{1}(ficha)=\pi_{1}(fichaSig)\land\pi_{2}(ficha)=\pi_{2}(fichaSig)$}
          \State\Return false
          \EndIf
          \State$\textsc{Avanzar}(oItSig)$
        \EndWhile
        \State$\textsc{Avanzar}(oIt)$
    \EndWhile
