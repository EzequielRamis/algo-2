...
\begin{adjustwidth}{3em}{}\ \
  ...
  \tupItem{jugadores}{array\_dimensionable(tupla(
    \begin{adjustwidth}{3em}{}\
        \textrm{$puntaje$:} nat\\
        \textrm{$fichasUbicadasUltimamente$:} ocurrencia\\
...
\Rep[juego\_estr]{$
  (\forall i:\texttt{nat})(i<tam(e.jugadores)\implies(
  $\begin{adjustwidth}{1em}{0em}
    $
    tam(e.jugadores[i].cantFichasPorLetra)=\textsc{fichasPorJugador}(e.variante)\land\\
    tam(e.jugadores[i].cantFichasPorLetra)=\textsc{dom}()\yluego\\
    \sum_{f<\textsc{dom}()}e.jugadores[i].cantFichasPorLetra[f]=e.jugadores[i].cantFichasTotal
    $
  \end{adjustwidth}$))\land\\
  tam(e.tablero)=\textsc{tamañoTablero}(e.variante)\land\\
  e.jugadorActual\leq tam(e.jugadores)\land\\
  (\forall i:\texttt{nat})(i<tam(e.tablero)\impluego tam(e.tablero[i])=tam(e.tablero))\yluego\\
  \sum_{i<tam(e.jugadores)}e.jugadores[i].puntaje\geq\\
  \sum_{i,j<tam(e.tablero)}\LIF\ e.tablero[i][j]\neq\textsc{NULL}$
